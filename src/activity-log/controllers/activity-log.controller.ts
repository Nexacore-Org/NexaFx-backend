...
import { LogoutResponseDto, SessionListResponseDto } from "../dto/activity-log-response.dto"
...
export class ActivityLogController {
  ...
  @ApiResponse({
    ...
    schema: {
      type: "object",
      properties: {
        activities: { type: "array", items: { $ref: "#/components/schemas/ActivityLogResponseDto" } },
  ...
  async getUserActivityHistory(request: Request, @Query('page') page: number = 1, @Query('limit') limit: number = 20) {
    const userId = request.user?.["id"]
  ...
  async logoutOtherSessions(request: Request): Promise<LogoutResponseDto> {
    ...
    return this.activityLogService.logoutOtherSessions(userId, currentSessionId)
  }
  ...
  async terminateSession(request: Request, @Param('sessionId') sessionId: string) {
    const userId = request.user?.["id"]
  ...
  async checkSuspiciousActivity(request: Request) {
    ...
    const isSuspicious = await this.activityLogService.checkSuspiciousActivity(userId, ipAddress)
