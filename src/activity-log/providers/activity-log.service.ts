...
export class ActivityLogService {
  ...
  async logLogoutActivity(
    ...
    try {
      ...
      this.logger.log(
        `User ${userId} logged out${sessionId ? ` from session ${sessionId}` : ''}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to log logout activity: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async logoutOtherSessions(
    ...
    try {
      ...
      this.logger.log(
        `User ${userId} logged out from ${result.affected} other sessions`,
      );
      ...
      this.logger.error(
        `Failed to logout other sessions: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async logoutAllSessions(userId: string): Promise<LogoutResponseDto> {
    try {
      ...
      this.logger.log(
        `User ${userId} logged out from all ${result.affected} sessions`,
      );
      ...
      this.logger.error(
        `Failed to logout all sessions: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getUserActivityHistory(
    ...
    try {
      ...
      const activityDtos: ActivityLogResponseDto[] = activities.map(
        (activity) => ({
          id: activity.id,
      ...
      this.logger.error(
        `Failed to get user activity history: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async checkSuspiciousActivity(
    ...
    try {
      ...
      if (!recentActivity) {
        this.logger.warn(
          `Suspicious activity detected: User ${userId} logging in from new IP ${ipAddress}`,
        );
        return true;
      }
      ...
      if (recentLogins > 10) {
        ...
        this.logger.warn(
          `Suspicious activity detected: User ${userId} has ${recentLogins} recent login attempts`,
        );
        return true;
      }
      ...
      this.logger.error(
        `Failed to check suspicious activity: ${error.message}`,
      );
      return false;
    }
  }
