...
export class ContractService {
    ...
    async executeContractFunction(executeDto: ExecuteContractDto) {
        try {
            this.logger.log(`Executing contract function: ${executeDto.functionName}`);

            const sourceKeypair = StellarSdk.Keypair.fromSecret(executeDto.sourceSecret);
            const sourceAccount = await this.server.loadAccount(sourceKeypair.publicKey());
    ...
    async getTransactionByHash(txHash: string) {
        try {
            ...
            const stellarTx = await this.server.transactions()
                .transaction(txHash)
                .call();
