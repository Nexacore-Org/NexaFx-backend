import { Controller, Post, Get, UseGuards, HttpStatus, HttpCode } from "@nestjs/common"
...
export class ConvertController {
  ...
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({ status: 422, description: "Insufficient balance or invalid conversion" })
  async convertCurrency(req: any, createConvertDto: CreateConvertDto): Promise<ConvertResponseDto> {
  ...
  @ApiQuery({ name: "fromCurrency", required: false, type: String, description: "Filter by source currency" })
  @ApiQuery({ name: "toCurrency", required: false, type: String, description: "Filter by target currency" })
  @ApiResponse({
    status: 200,
    description: "Conversion history retrieved successfully",
    type: [ConvertResponseDto],
  })
  async getConversionHistory(req: any, page = 1, limit = 10, fromCurrency?: string, toCurrency?: string) {
    const userId = req.user.sub
  ...
  @ApiQuery({ name: "base", required: false, type: String, description: "Base currency" })
  @ApiResponse({
    status: 200,
