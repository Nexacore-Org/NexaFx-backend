...
import { ConversionQuoteDto, ConversionQuoteResponseDto } from "../dto/conversion-quote.dto"
...
export class ConvertService {
  ...
  private readonly supportedCurrencies = ["NGN", "USD", "USDC", "BTC", "ETH", "USDT", "BNB"]

  // Mock exchange rates (in production, fetch from external API)
  private readonly exchangeRates = {
    NGN: { USD: 0.00062, USDC: 0.00062, BTC: 0.000000015, ETH: 0.00000025, USDT: 0.00062, BNB: 0.0000009 },
    USD: { NGN: 1610, USDC: 1, BTC: 0.000024, ETH: 0.0004, USDT: 1, BNB: 0.0015 },
    USDC: { NGN: 1610, USD: 1, BTC: 0.000024, ETH: 0.0004, USDT: 1, BNB: 0.0015 },
    BTC: { NGN: 67000000, USD: 41600, USDC: 41600, ETH: 16.5, USDT: 41600, BNB: 62 },
    ETH: { NGN: 4060000, USD: 2520, USDC: 2520, BTC: 0.061, USDT: 2520, BNB: 3.8 },
    USDT: { NGN: 1610, USD: 1, USDC: 1, BTC: 0.000024, ETH: 0.0004, BNB: 0.0015 },
    BNB: { NGN: 1073000, USD: 667, USDC: 667, BTC: 0.016, ETH: 0.26, USDT: 667 },
  }
  ...
  async getConversionQuote(quoteDto: ConversionQuoteDto): Promise<ConversionQuoteResponseDto> {
  ...
  async convertCurrency(userId: string, createConvertDto: CreateConvertDto): Promise<ConvertResponseDto> {
    const { fromCurrency, toCurrency, amount, description } = createConvertDto

    this.logger.log(`Converting ${amount} ${fromCurrency} to ${toCurrency} for user ${userId}`)
    ...
    try {
      ...
      const transaction = this.transactionRepository.create({
        ...
        reference,
        description: description || `Convert ${amount} ${fromCurrency} to ${convertedAmount} ${toCurrency}`,
        metadata: {
          fromCurrency,
          toCurrency,
      ...
      await this.updateUserBalances(user, fromCurrency, toCurrency, totalDeduction, convertedAmount)

      // Send notification
      await this.sendConversionNotification(user, Array.isArray(savedTransaction) ? savedTransaction[0] : savedTransaction)

      this.logger.log(`Conversion completed successfully: ${reference}`)

      return this.mapToConvertResponse(Array.isArray(savedTransaction) ? savedTransaction[0] : savedTransaction)
    } catch (error) {
      this.logger.error(`Conversion failed for user ${userId}:`, error)
      throw new UnprocessableEntityException("Conversion failed. Please try again.")
    }
  }
  ...
  async getConversionHistory(
    ...
    const queryBuilder = this.transactionRepository
      ...
      .andWhere("transaction.type = :type", { type: TransactionType.CONVERSION })
      .orderBy("transaction.createdAt", "DESC")
      .skip(skip)
      .take(limit)

    if (fromCurrency) {
      queryBuilder.andWhere("transaction.asset = :fromCurrency", { fromCurrency })
    }

    if (toCurrency) {
      queryBuilder.andWhere("JSON_EXTRACT(transaction.metadata, '$.toCurrency') = :toCurrency", { toCurrency })
    }

    const [transactions, total] = await queryBuilder.getManyAndCount()

    return {
      data: transactions.map((transaction) => this.mapToConvertResponse(transaction)),
      pagination: {
        page,
        limit,
        total,
  ...
  async getConversionById(userId: string, id: string): Promise<ConvertResponseDto> {
  ...
  private async validateUserBalance(user: User, currency: string, requiredAmount: number): Promise<void> {
  ...
  private async sendConversionNotification(user: User, transaction: Transaction): Promise<void> {
    try {
  ...
  private mapToConvertResponse(transaction: Transaction): ConvertResponseDto {
    ...
    return {
      ...
      feeCurrency: transaction.feeCurrency?.code || transaction.currency?.code || transaction.asset,
