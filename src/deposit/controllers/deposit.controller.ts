import { Controller, Post, Get, UseGuards, Query, Param, HttpStatus, HttpCode } from "@nestjs/common"
...
export class DepositController {
  ...
  async createDeposit(createDepositDto: CreateDepositDto, req: any): Promise<DepositResponseDto> {
  ...
  async generateWalletAddress(@Param('currency') currency: string, req: any): Promise<WalletAddressResponseDto> {
    const userId = req.user.sub
  ...
  async getDepositById(@Param('id') id: string, req: any): Promise<DepositResponseDto> {
    const userId = req.user.sub
  ...
  @ApiResponse({ status: 404, description: "Deposit not found" })
  async confirmDeposit(@Param('id') id: string, req: any): Promise<DepositResponseDto> {
    const userId = req.user.sub
