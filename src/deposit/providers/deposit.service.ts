...
import { DepositMethodsResponseDto, DepositResponseDto, WalletAddressResponseDto } from "../dto/deposit-response.dto"
...
export class DepositService {
  ...
  async createDeposit(userId: string, createDepositDto: CreateDepositDto): Promise<DepositResponseDto> {
    const { currency, amount, method, reference, description, sourceAddress } = createDepositDto
    ...
    const savedTransaction = await this.transactionRepository.save(depositTransaction)
    ...
    return this.mapToDepositResponse(savedTransaction, destinationAddress, qrCodeData)
  }
  ...
  async generateWalletAddress(userId: string, currency: string): Promise<WalletAddressResponseDto> {
    // Validate currency
    const currencyEntity = await this.currencyRepository.findOne({
      where: { code: currency.toUpperCase() },
    })
  ...
  async getDepositHistory(
    ...
    if (currency) {
      queryBuilder.andWhere("transaction.asset = :currency", { currency: currency.toUpperCase() })
    }
    ...
    const deposits = transactions.map((transaction) =>
      this.mapToDepositResponse(transaction, transaction.destinationAccount, transaction.metadata?.qrCodeData),
    )
  ...
  async getDepositById(id: string, userId: string): Promise<DepositResponseDto> {
    ...
    return this.mapToDepositResponse(transaction, transaction.destinationAccount, transaction.metadata?.qrCodeData)
  }

  async confirmDeposit(id: string, userId: string): Promise<DepositResponseDto> {
    const transaction = await this.transactionRepository.findOne({
      where: {
        id,
        initiatorId: userId,
  ...
  private async generateCryptoAddress(currency: string): Promise<string> {
    ...
    return mockAddresses[currency.toUpperCase()] || `${currency.toUpperCase()}-${uuidv4()}`
  }
