...
export class ScheduledTransfersService {
   ...
   */
  async create(userId: string, createDto: CreateScheduledTransferDto): Promise<ScheduledTransfer> {
    ...
    const scheduledTransfer = this.scheduledTransferRepository.create({
      ...
      reference: createDto.reference || `SCH-${Date.now()}-${userId.substring(0, 8)}`,
    ...
    try {
      ...
      this.logger.warn(`Failed to calculate fees for scheduled transfer: ${error.message}`)
    }

    const savedTransfer = await this.scheduledTransferRepository.save(scheduledTransfer)

    this.logger.log(`Created scheduled transfer ${savedTransfer.id} for user ${userId}`)
  ...
  async findOne(id: string, userId?: string): Promise<ScheduledTransfer> {
    ...
    if (userId && transfer.userId !== userId) {
      throw new ForbiddenException("You do not have permission to access this scheduled transfer")
    }
   ...
   */
  async update(id: string, userId: string, updateDto: UpdateScheduledTransferDto): Promise<ScheduledTransfer> {
    const transfer = await this.findOne(id, userId)

    // Prevent updates to executed or cancelled transfers
    if (transfer.status === ScheduledTransferStatus.EXECUTED || transfer.status === ScheduledTransferStatus.CANCELLED) {
      throw new ConflictException(`Cannot update a transfer that has been ${transfer.status.toLowerCase()}`)
    }
    ...
    if (updateDto.scheduledAt && new Date(updateDto.scheduledAt) <= new Date()) {
      throw new BadRequestException("Scheduled date must be in the future")
    }
   ...
   */
  async cancelTransfer(transfer: ScheduledTransfer): Promise<ScheduledTransfer> {
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (transfer.status !== ScheduledTransferStatus.PENDING) {
      throw new ConflictException(`Cannot delete a transfer that has been ${transfer.status.toLowerCase()}`)
    }
   ...
   */
  async executeTransfer(transfer: ScheduledTransfer): Promise<ScheduledTransfer> {
    try {
      ...
      this.logger.log(`Successfully executed scheduled transfer ${transfer.id} as transaction ${transaction.id}`)
      ...
      transfer.failureReason = error.message || "Unknown error during execution"

      this.logger.error(`Failed to execute scheduled transfer ${transfer.id}: ${transfer.failureReason}`, error.stack)
  ...
  async processScheduledTransfers() {
    ...
    try {
      ...
      this.logger.log(`Found ${pendingTransfers.length} scheduled transfers to execute`)
      ...
      for (const transfer of pendingTransfers) {
        try {
          ...
          this.logger.error(`Error executing scheduled transfer ${transfer.id}: ${error.message}`, error.stack)
        }
      }
    } catch (error) {
      this.logger.error(`Error processing scheduled transfers: ${error.message}`, error.stack)
    }
  }
  ...
  async getStats(userId?: string) {
    const queryBuilder = this.scheduledTransferRepository.createQueryBuilder("transfer")
  ...
  async executeNow(id: string, userId: string): Promise<ScheduledTransfer> {
    ...
    if (transfer.status !== ScheduledTransferStatus.PENDING) {
      throw new ConflictException(`Cannot execute a transfer that is already ${transfer.status.toLowerCase()}`)
    }
