...
export class WithdrawService {
  ...
  async createWithdrawal(userId: string, createWithdrawalDto: CreateWithdrawalDto): Promise<WithdrawalResponseDto> {
    const { currency, amount, destination, method, description } = createWithdrawalDto
    ...
    const withdrawal = this.transactionRepository.create({
      ...
      reference,
      description: description || `Withdrawal to ${method === WithdrawalMethod.WALLET ? "wallet" : "fiat"}`,
  ...
  async getWithdrawalHistory(
    ...
    page = 1,
    limit = 10,
  ): Promise<{ withdrawals: WithdrawalResponseDto[]; total: number; page: number; limit: number }> {
    ...
    return {
      withdrawals: withdrawals.map((withdrawal) => this.mapToResponseDto(withdrawal)),
      total,
      page,
      limit,
    }
  ...
  async getWithdrawalById(id: string, userId: string): Promise<WithdrawalResponseDto> {
  ...
  private async sendWithdrawalNotification(user: User, withdrawal: Transaction): Promise<void> {
    try {
